...
boolean_var_names = ('vars.autoBalance', 'vars.friendlyFire', 'vars.killCam', 'vars.miniMap', 'vars.hud', 'vars.3dSpotting',
                     'vars.nameTag', 'vars.3pCam', 'vars.regenerateHealth', 'vars.vehicleSpawnAllowed', 'vars.onlySquadLeaderSpawn',
                     'vars.premiumStatus', 'reservedSlotsList.aggressiveJoin')
...
class boolean_test_maker_type(type):
    ...
    def nominal_test(var_name):
        def f(self):
            for data, expected in [('true','true'), ('false','false'), ('1','true'), ('0','false')]:
                # set new data
                self.cmd('%s' % var_name, data)
                # verify new data is correctly set
                actual, = self.cmd('%s' % var_name)
                self.assertEqual(expected, actual, "%s '%s' failed, expecting '%s', got '%s' instead" % (var_name, data, expected, actual))
        return f
    ...
    def bad_data_test(var_name):
        def f(self):
            try:
                self.cmd('%s' % var_name, 'fo0')
            except CommandFailedError, err:
                self.assertEqual(['InvalidArguments'], err.message, "%s 'fo0' expected to fail with error 'InvalidArguments', got '%r' instead" % (var_name, err.message))
            else:
                self.fail("expecting InvalidArguments error for : %s 'fo0'" % var_name)
        return f
    ...
    def too_many_arguments(var_name):
        def f(self):
            try:
                self.cmd('%s' % var_name, 'true', 'fo0')
            except CommandFailedError, err:
                self.assertEqual(['InvalidArguments'], err.message, "%s 'true' 'fo0' expected to fail with error 'InvalidArguments', got '%r' instead" % (var_name, err.message))
            else:
                self.fail("expecting InvalidArguments error for : %s 'true' 'fo0'" % var_name)
        return f
